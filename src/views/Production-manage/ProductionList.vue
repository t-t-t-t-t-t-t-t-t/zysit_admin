<template>
    <div>
        <el-card>
            <template #header>
                <div class="card-header" style="display:flex;justify-content: space-between;">
                    <span>产品列表</span>
                    <el-button @click="isShowAddBox = true">添加产品</el-button>
                </div>
            </template>
        ...
        <ProductAdd @add="onHandleAddProduct" @closeBox='isShowAddBox = false' :isShowDialog="isShowAddBox">
        </ProductAdd>
        <ProductEdit @edit="onHandleEditProduct" @closeBox="isShowEditBox = false" :isShowDialog="isShowEditBox"
            :formData="tableData[currentIdx]"></ProductEdit>
    </div>
</template>
...
const handleDelete = (index, data) => {
    useProductHooks().deleteProduct(data._id).then(res => {
...
const onHandleEditProduct = (product) => {
    tableData.value.splice(currentIdx.value, 1, product)
}

onMounted(() => {
    useProductHooks().getProductList().then(res => {
        tableData.value = res
    })
})
</script>
