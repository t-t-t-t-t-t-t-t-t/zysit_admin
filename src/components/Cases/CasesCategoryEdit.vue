<template>
    <el-dialog :model-value="isShowDialog" @submit.prevent title="编辑应用案例类别" width="500" @close=" emit('closeBox')">
...
const emit = defineEmits(['editCasesCategory', 'closeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useCasesCategoryHooks().putCasesCategory(formData).then(res => {
                emit('editCasesCategory', { _id: formData._id, title: formData.title })
                emit('closeBox')
