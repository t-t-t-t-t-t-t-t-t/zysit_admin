...
const emit = defineEmits(['addSolutionCategory', 'changeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useSolutionCategoryHooks().addSolutionCategory(formData).then(res => {
                emit('addSolutionCategory', { _id: res._id, title: res.title })
                emit('closeBox')
