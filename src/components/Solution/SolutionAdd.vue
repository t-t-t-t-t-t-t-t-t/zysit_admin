<template>
    <el-dialog :model-value="isShowDialog" @submit.prevent title="添加解决方案类别" width="500" @close="emit('closeBox')">
...
const emit = defineEmits(['addSolution', ' closeBox'])
const { isShowDialog } = defineProps(['isShowDialog'])

const formRef = ref()
const formData = reactive({ title: "", bannerImage: "", category: {}, mcontent: "", contentImage: "", demandAnalysis: [], heighLight: [], mainProduct: [], mainCaseImage: "", mainCase: [] })
...
const fromRules = reactive({//表单校验
    ...
    mainProduct: [{
        required: false, message: "请输入主要产品",
        trigger: "blur"// 出发校验的条件
    }],
    mainCaseImage: [{
...
const submitForm = () => {
    formRef.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useSolutionHooks().addSolution(formData).then(res => {
                emit('addSolution', { _id: res._id, title: res.title, category: res.category, bannerImage: res.bannerImage, content: res.content, contentImage: res.contentImage, demandAnalysis: res.demandAnalysis, heighLight: res.heighLight, mainProduct: res.mainProduct, mainCaseImage: res.mainCaseImage, mainCase: res.mainCase })
                emit('closeBox')
