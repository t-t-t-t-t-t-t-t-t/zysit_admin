...
const emit = defineEmits(['editSolutionCategory', 'changeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useSolutionCategoryHooks().putSolutionCategory(formData).then(res => {
                emit('editSolutionCategory', { _id: formData._id, title: formData.title })
                emit('closeBox')
                if (!formRed.value) return
                formRed.value.resetFields()
                onHandleChangeDialog();
            })
        }
    }).catch(err => {
        console.log(err)
