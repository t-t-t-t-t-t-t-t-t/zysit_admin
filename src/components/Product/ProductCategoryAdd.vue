<template>
    <el-dialog :model-value="isShowDialog" @submit.prevent title="添加产品类别" width="500" @close=" emit('closeBox')">
...
const emit = defineEmits(['addProductCategory', 'closeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useProductCategoryHooks().addProductCategory(formData).then(res => {
                emit('addProductCategory', { _id: res._id, title: res.title })
                emit('closeBox')
