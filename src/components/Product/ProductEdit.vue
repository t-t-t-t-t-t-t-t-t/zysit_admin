<template>
    <el-dialog :model-value="isShowDialog" title="编辑产品列表" width="500" @close="emit('closeBox')">
        <el-form :model="formData" ref="formRed" label-width="auto" :rules="fromRules" style="max-width: 600px">
            ...
            <el-form-item label="参数" prop="parameters">
                <div class="flex gap-2">
                    ...
                    <el-input v-if="inputValueVisible" ref="InputValueRef" v-model="inputValueValue" placeholder="请输入值"
                        class="w-20" size="small" @keyup.enter="handleInputValueConfirm"
...
const emit = defineEmits(['edit', 'closeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useProductHooks().putProduct(formData).then(res => {
                emit('edit', { _id: formData._id, title: formData.title, image: formData.image, content: formData.content, isXin: formData.isXin, isHot: formData.isHot, parameters: formData.parameters, scene: formData.scene, category: formData.category })
                emit('closeBox')
