<template>
    <el-dialog :model-value="isShowDialog" title="添加荣誉" @submit.prevent width="500" @close=" emit('closeBox')">
...
const emit = defineEmits(['editHoner', 'closeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useHonerHooks().putHoner(formData).then(res => {
                emit('editHoner', { _id: formData._id, title: formData.title, image: formData.image })
                emit('closeBox')
