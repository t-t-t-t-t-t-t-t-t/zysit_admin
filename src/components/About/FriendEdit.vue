<template>
    <el-dialog :model-value="isShowDialog" @submit.prevent title="编辑合作伙伴" width="500" @close=" emit('closeBox')">
...
const emit = defineEmits(['editFriend', 'closeBox'])
...
const submitForm = () => {
    formRed.value.validate().then(valid => {
        if (valid) {//通过校验
            // 提交到后台
            useFriendHooks().putFriend(formData).then(res => {
                emit('editFriend', { _id: formData._id, title: formData.title, image: formData.image })
                emit('closeBox')
